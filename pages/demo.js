...
export default function Demo() {
  ...
  useEffect(() => {
    if (txHash) {
      toast(
        <BlockExplorerLink txHash={txHash} />,
        {
  ...
  // We want to generate the wallet only once so we do so
  // within a useEffect() hook, without any dependencies so it doesn't re-run
  useEffect(() => {
